...
USER_NAME = os.environ["USER_NAME"]  # 'Akhilesh9911'
QUERY_COUNT = {"user_getter": 0, "graph_repos_stars": 0, "recursive_loc": 0, "graph_commits": 0, "loc_query": 0, "follower_getter": 0}
...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    try:
        ...
        data = []
        if comment_size > 0:
            for _ in range(comment_size): data.append("comment\n")
...
if __name__ == "__main__":
    """
    Akhilesh Chitare (Akhilesh9911) - GitHub Profile README generator
    Adapted from Andrew Grant's (Andrew6rant) neofetch-style README
    """
    print("Calculation times:")
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    OWNER_ID, acc_date = user_data
    print(f"   account data:          {user_time*1000:.4f} ms")
    ...
    svg_overwrite("light_mode.svg", commit_data, star_data, repo_data, follower_data, total_loc[:-1])
